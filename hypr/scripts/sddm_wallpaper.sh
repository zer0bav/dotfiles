...
wallpaper_picker="$HOME/.config/hypr/UserScripts/WallpaperSelect.sh"
...
# Ensure we have a usable wallpaper file. If missing, pick one now.
if [[ ! -s "$wallpaper_path" ]]; then
    notify-send -i "$iDIR/wallpaper.png" "SDDM" "No wallpaper found, opening selector"
    if [[ -x "$wallpaper_picker" ]]; then
        "$wallpaper_picker"
    fi
fi

if [[ ! -s "$wallpaper_path" ]]; then
    notify-send -i "$iDIR/error.png" "SDDM" "Wallpaper file not found: $wallpaper_path"
    exit 1
fi
